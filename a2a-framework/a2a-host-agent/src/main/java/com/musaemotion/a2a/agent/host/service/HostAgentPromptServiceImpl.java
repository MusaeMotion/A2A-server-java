...
public class HostAgentPromptServiceImpl implements HostAgentPromptService {
    ...
    /**
     * 根提示词， check_pending_task_states 说明已经干掉，最新代码也没有实现，感觉暂时不需要这个状态
     */
     ...
     * 提示词列表字符串
     */
    private String LIST_AGENT_PROMPT_TPL = " name:%s, description: %s \n ";
    ...
    public String loadRemoteAgentsToString() {
        ...
        this.loadRemoteAgents().forEach(agentInfo -> {
            sb.append(
                    String.format(LIST_AGENT_PROMPT_TPL, agentInfo.getName(), agentInfo.getDescription())
            );
        });
        return sb.toString();
    }
