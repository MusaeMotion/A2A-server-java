...
public class PartUtils {
    ...
    public static String getTextContent(Common.Message message) {
        ...
        if(op.isPresent()) {
            ...
            Map<String, Object> fileMap = getFilePartMap(message);
            if(fileMap.size()>0){
     ...
     * @return
     */
    public static List<Common.Part> filterPartNoText(Common.Message message) {
       return message.getParts().stream().filter(part -> !(part instanceof Common.TextPart)).collect(Collectors.toUnmodifiableList());
    }
     ...
     * @return
     */
    public static List<Common.Part> getFilePart(Common.Message message) {
     ...
     * @return
     */
    private static Map<String, Object> getFilePartMap(Common.Message message) {
        var op = message.getParts().stream().filter(item->item instanceof Common.FilePart).findFirst();
        if(op.isPresent()) {
